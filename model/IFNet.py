...
class VSRbackbone(nn.Module):
        ...
        def forward(self, allframes,  scale=[4,2,1], timestep=0.5):
            # using septuplet: 7*3*224*224 => 21*224*224
            c, h, w = allframes.size()
            outputs = []
            # backward propgation
            feat_prop = allframes.new_zeros(n, self.mid_channels, h, w)
            for i in range(3 -1, -1, -1):
                print("img:",3*t,3*t+1,3*t+2)
                if i < t - 1:  # no warping required for the last timestep
                    img0 = allframes[:, 6*i,6*i+3]
                    img1 = allframes[:, 6*i+3,6*i+6]
                    gt = allframes[:, 6*i,6*i+3] # In inference time, gt is None
